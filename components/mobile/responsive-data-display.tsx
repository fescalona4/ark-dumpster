...
export function ResponsiveDataDisplay({ 
  ...
  const handleSwipeLeft = React.useCallback((orderId: number) => {
  ...
  const handleSwipeRight = React.useCallback((orderId: number) => {
  ...
  const handleOrderTap = React.useCallback((order: OrderData) => {
    onOrderSelect?.(order);
  }, [onOrderSelect]);
  ...
  return (
    <div className="flex overflow-x-auto pb-2 px-4 gap-2 scrollbar-hide">
      {allStatuses.map((status) => (
        ...
        >
          <span>
            {status === 'all' ? 'All' : status.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
          </span>
          {orderCounts[status] && (
